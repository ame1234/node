...
class Worker extends EventEmitter {
  constructor(filename, options = {}, internalMarker) {
    super();
    const internal = internalMarker === InternalWorker;
    ...
    if (!internal) {
      const loaderRPC =
        require('internal/modules/esm/loader').getBottomLoaderRPC();
      if (loaderRPC && loaderRPC.addBelowPort) {
        ({
          port1: outsideBelowPort,
          port2: insideBelowPort
        } = new MessageChannel());
        loaderRPC.addBelowPort({
