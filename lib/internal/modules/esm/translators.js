...
translators.set('module', async function moduleStrategy(url) {
  let { source } = await this._getSource({ url, format: 'module' });
  source = `${source}`;
  ({ source } = await this._transformSource({ source, url, format: 'module' }));
...
translators.set('json', async function jsonStrategy(url) {
  ...
  let { source } = await this._getSource({ url, format: 'json' });
  source = `${source}`;
  ({ source } = await this._transformSource({ source, url, format: 'json' }));
  if (pathname) {
...
translators.set('wasm', async function(url) {
  ...
  let { source } = await this._getSource({ url, format: 'wasm' });
  ({ source } = await this._transformSource({ source, url, format: 'wasm' }));
