...
class Loader {
  ...
  async resolve(specifier, parentURL) {
    ...
    const resolveResponse = bottomLoaderRPC ? await bottomLoaderRPC.send(new ResolveRequest({
      clientId: threadId,
      specifier,
      base: parentURL
    ...
    const TODO_FIXED_FORMAT_RPC = false;
    const getFormatResponse = (bottomLoaderRPC && TODO_FIXED_FORMAT_RPC) ?
      await bottomLoaderRPC.send(new GetFormatRequest({
        clientId: threadId,
        url,
      })) : defaultGetFormat(url, {}, defaultGetFormat);
  ...
  // hook({ resolve, getFormat, getSource, transformSource }) {
