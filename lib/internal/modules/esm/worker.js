...
const defaultLoaderWorker = {
  resolveImportURL(params) {
    return defaultResolve(params.specifier, { parentURL: params.base });
  },
...

{
  ...
  async function forwardResolveHook(specifier, options) {
    return parentLoaderAPI.resolveImportURL({ specifier, base: options.parentURL });
  }
  ...
  const userLoaderWorker = {
    async resolveImportURL(params) {
      ...
      const options = { parentURL: params.base };
