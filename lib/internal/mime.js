...
function readUntilCharacter(str, char) {

}
function readUntilPattern(str, pattern) {
  return StringPrototypeSearch(str, pattern);
}
...
function parseTypeAndSubtype(str) {
  ...
  // read until '/'
  const typeEnd = StringPrototypeIndexOf(
    SOLIDUS,
    StringPrototypeSlice(str));
  const trimmedType = typeEnd === -1 ?
    str :
    StringPrototypeSlice(str, typeEnd);
  if (trimmedType === '' ||
  ...
  // read until ';'
  const subtypeEnd = StringPrototypeIndexOf(
    SEMICOLON,
    StringPrototypeSlice(str, position));
  const rawSubtype = subtypeEnd === -1 ?
    StringPrototypeSlice(position) :
    StringPrototypeSlice(position, subtypeEnd);
