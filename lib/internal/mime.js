...
function parse(str, target) {
  ...
  let position = 0;
  let type = execPattern(/^[^\/]*/, str);
  if (!type || type[0] === '' || testPattern(NotHTTPTokenCodePoint, type[0])) {
    throw new TypeError('Error parsing MIME: invalid type');
  }
  type = type[0];
  position = type.length;
  position += 1;
  let subtype = execPattern(/^[^;]*/, StringPrototypeSlice(str, position));
  if (subtype) {
    position = position += subtype[0].length;
    subtype[0] = StringPrototypeReplace(subtype[0], ENDING_WHITESPACE, '');
  }
  if (!subtype || subtype[0] === '' ||testPattern(NotHTTPTokenCodePoint, subtype[0])) {
  ...
  while (position < str.length) {
    position++;
    const skip = execPattern(BEGINNING_WHITESPACE, StringPrototypeSlice(str, position));
    if (skip) position += skip[0].length;
    let parameterName = execPattern(/^[^;=]*/, StringPrototypeSlice(str, position));
    ...
    if (codePoint === '"') {
      position += 1;
      const inside = execPattern(/^(?:([\\]$)|[\\][\s\S]|[^"])*(?:(")|$)/u, StringPrototypeSlice(str, position));
      ...
      let value = execPattern(/[^;]*/, StringPrototypeSlice(str, position))[0];
    ...
    if (parameterName !== '' &&
      testPattern(NotHTTPTokenCodePoint, parameterName) === false &&
